...
// Maps to:    {projectsRoot}/{project}/artifacts/{feature}/{name}.md on disk
type ArtifactResources struct {
	projectsRoot string
}

func NewArtifactResources(cfg config.AppConfig) *ArtifactResources {
	return &ArtifactResources{projectsRoot: cfg.Dirs.ProjectsRoot}
}
...
func (a *ArtifactResources) read(_ context.Context, req mcp.ReadResourceRequest) ([]mcp.ResourceContents, error) {
	...
	path := filepath.Join(a.projectsRoot, project, "artifacts", feature, name+".md")
